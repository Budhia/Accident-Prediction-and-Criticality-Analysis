...
demo_path = './dataset/features/testing/'#training/'
default_model_path = './model/final_model'
save_path = './model/'
video_path = './dataset/videos_1/testing/'#positive/'
# batch_number
train_num = 126
test_num = 46
...
def plotting(i,pred,cnt):
    plt.figure(figsize=(14,5))
    plt.axhline(y=0.4,linewidth=2.0, color='red')
    plt.plot(pred[i,0:99],linewidth=3.0)
    plt.ylim(0, 1)
    mask = ma.masked_less(pred[i,0:99], 0.4)    
    plt.plot(mask,linewidth=3.0, color='red')
    plt.ylim(0,1)  
...
def vis(model_path):
    ...
    print('\nBatch NUmber :: ',num_batch,'\n')
    file_name = '%03d' %num_batch
    #f_name='%06d' %(10*test_num+num_batch)
    ...
    for i in range(len(ID)):
        if labels[i][1] == 1 :
            file_name='%06d'%int(ID[i])
            print('TESTING:'+video_path+ file_name +'.mp4') 
            #file_name = ID[i]
            print(file_name)
            bboxes = det[i]
            ...
            while(ret):
                ...
                #print(counter+1,'::', pred[i,counter])
                if(pred[i,counter]<0.4):
                    cv2.putText(dst,str(counter+1),(10,30), font, 1,(255,255,255),3)
                else:
                    cv2.putText(dst,str(counter+1),(10,30), font, 1,(255,255,255),3)
                    cv2.putText(dst,'ALERT',(200,200), font, 5,(0,0,255),3)
                out.write(dst)
                cv2.imshow('TESTING:'+video_path+ file_name +'.mp4',dst)
                cv2.imwrite('Images/frame_'+str(fr)+'.jpg',dst)
                fr+=1
                c = cv2.waitKey(50)
                ret, frame = cap.read()
